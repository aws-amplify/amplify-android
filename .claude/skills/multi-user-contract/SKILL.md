...
**`setAuthState` userId recovery (gated on explicit userId for the reset).** `StateMachineForAuth.setAuthState(userId, value)` always emits to `_state`. Persistence to `AuthStateRepo` happens whenever an effective userId can be determined — either the caller passed one, or it falls back to `signedInData.userId` when the new state is `SessionEstablished` (covers first sign-in, OAuth/WebUI redirect, process-death recovery). **The reset-to-default emission, however, only fires when the caller passed a non-empty userId** — that's the explicit opt-in to multi-user "stack the next user on top of the first." Single-user observers (no userId, even when the state is recoverable) keep the `SessionEstablished` emission live so `getCurrentState()` reports `SignedIn`, matching upstream `AuthStateMachine` semantics. This split replaces 2.26.14's separate `pendingSignInUserId` persistence (planned as E6/E7 in the port plan) and preserves backward compat for upstream-style single-user callers.

**Encrypted-store fallback.** `AuthStateRepo` wraps the `EncryptedKeyValueRepository` factory in a try/catch on `Exception` + `LinkageError` and falls back to a process-local `InMemoryFallbackStore` if the keystore can't initialize — covers corrupted keystore in production and JVM unit tests that lack `androidx.security.crypto.MasterKeys`. Multi-user routing keeps working in-memory; persistence is silently lost.

**`SignOutEvent` variants carry explicit userId** matching 2.26.14 §C.2. Each of `InvokeHostedUISignOut`, `SignOutLocally`, `SignOutGlobally`, `RevokeToken`, `SignOutGloballyError` exposes a nullable `userId: String? = null`. `AuthenticationCognitoActions.initiateSignOutAction` forwards `event.signOutData.userId` (set by `SignOutUseCase`); `SignOutCognitoActions` chains the userId through every downstream event in the sign-out action graph; `WebUiSignInResponseUseCase` populates the userId from `signedInData.userId` on its dispatched events. Action-layer code reads userId directly from the event payload without reaching into `signedInData` — invariant 5 holds literally and not just in spirit.
