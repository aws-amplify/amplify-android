...
internal class GraphQLHttpClient(
    ...
    /**
     * Execute a GraphQL request (query or mutation) with multi-auth retry.
     * For [AppSyncGraphQLRequest] (model-based), tries each auth mode from `@auth` rules.
     * For raw requests, uses the default auth mode.
     */
    suspend fun <T> execute(request: GraphQLRequest<T>): GraphQLResponse<T> =
        suspendCancellableCoroutine { continuation ->
            val operation = MultiAuthAppSyncGraphQLOperation.builder<T>()
