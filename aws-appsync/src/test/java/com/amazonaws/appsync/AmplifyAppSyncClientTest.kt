...
class AmplifyAppSyncClientTest {
    ...
    fun `the retry actually switches identity, it does not resend the same credentials`() = runTest {
        ...
        first.headers["authorization"] shouldBe "user-pools-token"
        first.headers["x-api-key"].shouldBeNull()
        second.headers["x-api-key"] shouldBe "da2-fakekey"
        second.headers["authorization"].shouldBeNull()
    }
    ...
    fun `a failing token supplier retries with the next auth mode instead of failing`() = runTest {
        ...
        server.requestCount shouldBe 1
        server.awaitRequest().headers["x-api-key"] shouldBe "da2-fakekey"
    }
