...
import com.amplifyframework.auth.cognito.actions.InitFetchIdentityAction
...
sealed class FetchIdentityState : State {
    ...
    class Resolver : StateMachineResolver<FetchIdentityState> {
            ...
            return when (oldState) {
                is Configuring -> {
                    when (fetchIdentityEvent) {
                        is FetchIdentityEvent.EventType.Fetch -> onFetchIdentity()
                        is FetchIdentityEvent.EventType.Fetched -> onFetchIdentitySuccess()
                ...
                is Fetching -> {
                    when (fetchIdentityEvent) {
                        is FetchIdentityEvent.EventType.Fetched -> onFetchIdentitySuccess()
                        is FetchIdentityEvent.EventType.ThrowError -> onFetchIdentityFailure()
                        else -> StateResolution(oldState)
                    }
                }
                is Fetched -> {
        ...
        private fun onFetchIdentitySuccess(): StateResolution<FetchIdentityState> {
            val newState = Fetched()
            return StateResolution(newState)
        }

        private fun onFetchIdentityFailure(): StateResolution<FetchIdentityState> {
            val newState = Error()
            return StateResolution(newState)
        }

        private fun onFetchIdentity(): StateResolution<FetchIdentityState> {
            val newState = Fetching()
            val action = InitFetchIdentityAction()
