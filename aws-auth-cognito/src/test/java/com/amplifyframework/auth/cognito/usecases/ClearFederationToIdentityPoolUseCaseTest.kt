...
class ClearFederationToIdentityPoolUseCaseTest {
    ...
    @Test
    fun `calls sign out if federated sign in`() = runTest {
        useCase.execute()

        coVerify {
            signOut.completeSignOut(withAuthEvent<AuthenticationEvent.EventType.ClearFederationToIdentityPool>(), any())
        }
    }

    @Test
    fun `calls sign out if error state for federated sign in`() = runTest {
