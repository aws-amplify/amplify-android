...
import com.amplifyframework.testmodels.customprimarykey.Blog;
 ...
 */
public final class BasicCloudSyncInstrumentationCPKTest {
    ...
    public static void setup() throws AmplifyException {
        ...
        @RawRes int configResourceId = Resources.getRawResourceId(context, "amplifyconfigurationcpk");
...
//                .syncExpression(Blog.class, () -> Blog.cr.gt(tenMinutesAgoDateTime))
//                .syncExpression(Post.class, () -> Post..gt(tenMinutesAgoDateTime))
...
//                .syncExpression(PostAuthorJoin.class, () -> PostAuthorJoin.CREATED_AT.gt(tenMinutesAgoDateTime))
    ...
    public void syncDownFromCloudIsWorkingWithCustomPrimaryKey() throws AmplifyException {
        ...
        GraphQLResponse<ModelWithMetadata<Blog>> createBlogResponse =
                appSync.create(blog, ModelSchema.fromModelClass(Blog.class));
        HubAccumulator receiptBlogAccumulator =
                HubAccumulator.create(HubChannel.DATASTORE, receiptOf(blog.getPrimaryKeyString()), 1)
                        .start();
        receiptBlogAccumulator.await(6000, TimeUnit.SECONDS);
        Post post = Post.builder()
                .title("test post")
                .blog(blog)
                .build();

        appSync.create(post, ModelSchema.fromModelClass(Post.class));
        HubAccumulator receiptPostAccumulator =
                HubAccumulator.create(HubChannel.DATASTORE, receiptOf(post.getPrimaryKeyString()), 1)
                        .start();
        receiptPostAccumulator.await(6000, TimeUnit.SECONDS);
        ...
        HubAccumulator receiptAccumulator =
                HubAccumulator.create(HubChannel.DATASTORE, receiptOf(comment.getPrimaryKeyString()), 1)
                        .start();

        // Act: create the model in the cloud
        ModelSchema schema = ModelSchema.fromModelClass(Comment.class);
        GraphQLResponse<ModelWithMetadata<Comment>> createResponse = appSync.create(comment, schema);
        ...
        receiptAccumulator.awaitFirst(6000, TimeUnit.SECONDS);

        // Jameson should be in the local DataStore.
        Comment result = dataStore.get(Comment.class, comment.getPrimaryKeyString());
        assertEquals(comment.getContent(), result.getContent());
        assertEquals(comment.getDescription(), result.getDescription());
        assertEquals(comment.getTitle(), result.getTitle());
    }
