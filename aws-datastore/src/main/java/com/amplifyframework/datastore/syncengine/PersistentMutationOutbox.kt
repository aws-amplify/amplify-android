 ...
 */
internal class PersistentMutationOutbox(localStorageAdapter: LocalStorageAdapter) : MutationOutbox {
    private val storage = Objects.requireNonNull(localStorageAdapter)
    ...
    override fun <T : Model> enqueue(incomingMutation: PendingMutation<T>): Completable {
        ...
        return Completable.defer {
            ...
            val modelClass = incomingMutation.modelSchema.modelClass.name
        ...
        return Completable.defer {
            ...
            Maybe.create { subscriber: MaybeEmitter<OutboxEvent> ->
                storage.delete(
                    ...
                    StorageItemChange.Initiator.SYNC_ENGINE,
                    QueryPredicates.all(),
                    { ignored: StorageItemChange<PersistentRecord?>? ->
     ...
     */ constructor(private val existing: PendingMutation<T>, private val incoming: PendingMutation<T>) {
        ...
        fun resolve(): Completable {
            ...
            return when (incoming.mutationType) {
                ...
                else -> unknownMutationType(existing.mutationType)
            }
        }
