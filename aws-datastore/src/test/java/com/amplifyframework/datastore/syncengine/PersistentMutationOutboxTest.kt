...
class PersistentMutationOutboxTest {
    private val schema = ModelSchema.fromModelClass(BlogOwner::class.java)
    private var converter = GsonPendingMutationConverter()
    ...
    fun enqueuePersistsMutationAndNotifiesObserver() {
        ...
        Assert.assertTrue(hasPendingMutation(jameson.id, jameson.javaClass.name))
    ...
    fun loadPreparesOutbox() {
        ...
        Assert.assertTrue(hasPendingMutation(tony.id, tony.javaClass.name))
        Assert.assertTrue(hasPendingMutation(sam.id, sam.javaClass.name))
    ...
    fun removeRemovesChangesFromQueue() {
        ...
        Assert.assertNull(mutationOutbox.peek())
        Assert.assertFalse(hasPendingMutation(bill.id, bill.javaClass.name))
    }
    ...
    fun hasPendingMutationReturnsTrueForExistingModelMutation() {
        ...
        val completed = mutationOutbox.enqueue(pendingMutation)
            .blockingAwait(TIMEOUT_MS, TimeUnit.MILLISECONDS)
        Assert.assertTrue(completed)
        Assert.assertTrue(hasPendingMutation(modelId, joe.javaClass.name))
        Assert.assertFalse(
            hasPendingMutation(
                mutationId.toString(),
                mutationId.javaClass.name
            )
        )
    }
    ...
    fun hasPendingMutationReturnsFalseForItemNotInStore() {
        ...
        Assert.assertFalse(hasPendingMutation(joeId, joe.javaClass.name))
        Assert.assertFalse(
            hasPendingMutation(
                unrelatedMutation.mutationId.toString(),
                unrelatedMutation.javaClass.name
            )
        )
    }
    ...
    fun hasPendingMutationReturnsFalseForModelMutationWithSamePrimaryKeyForDifferentModels() {
        ...
        Assert.assertTrue(hasPendingMutation(modelId, blogOwner.javaClass.name))
        ...
        Assert.assertFalse(hasPendingMutation(modelId, author.javaClass.name))
        ...
        Assert.assertTrue(hasPendingMutation(modelId, author.javaClass.name))
        ...
        Assert.assertFalse(hasPendingMutation(modelId, post.javaClass.name))
        ...
        Assert.assertTrue(hasPendingMutation(modelId, post.javaClass.name))
    }
    ...
    fun mutationEnqueueForModelWithDuplicatePrimaryKeyThrowsDatastoreException() {
        ...
        Assert.assertTrue(
            hasPendingMutation(
                ...
                existingBlogOwner.javaClass.name
            )
        )
    ...
    fun existingCreationIncomingCreationYieldsError() {
        ...
        Assert.assertTrue(
            hasPendingMutation(
                ...
                modelInExistingMutation.javaClass.name
            )
        )
        Assert.assertEquals(existingCreation, mutationOutbox.peek())
    }
    ...
    fun existingUpdateIncomingUpdateWithConditionAppendsMutation() {
        ...
        val existingStoredMutation = converter.fromRecord<BlogOwner>(
            recordsForExistingMutationId[0]
        )
    ...
    fun nextItemForModelIdReturnsFirstEnqueued() {
        ...
        Assert.assertEquals(
            firstMutation,
            mutationOutbox.getMutationForModelId(originalJoe.id, originalJoe.javaClass.name)
        )
    }
