...
class KinesisRecordSenderTest {
    ...
    fun `createRequest should construct correct PutRecordsRequest`() = runTest {
        // Given
        val recordSender = KinesisRecordSender(DummyCredentialsProvider(), testRegion, maxRetries)
    ...
    private class DummyCredentialsProvider : AWSCredentialsProvider<AWSCredentials> {
        override fun fetchAWSCredentials(onSuccess: Consumer<AWSCredentials>, onError: Consumer<AuthException>) {
            onSuccess.accept(AWSCredentials("dummy", "dummy"))
        }
    }
}
