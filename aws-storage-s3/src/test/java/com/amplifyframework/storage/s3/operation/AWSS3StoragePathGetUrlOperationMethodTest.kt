...
class AWSS3StoragePathGetUrlOperationMethodTest {
    ...
    fun `GET method calls getPresignedUrl`() {
        ...
        verify { storageService.getPresignedUrl(expectedServiceKey, expectedExpires, false) }
    ...
    fun `PUT method calls getPresignedUploadUrl`() {
        ...
        // THEN
        verify(exactly = 0) { onError.accept(any()) }
        verify { storageService.getPresignedUploadUrl(expectedServiceKey, expectedExpires, false) }
    ...
    fun `PUT method with accelerate endpoint`() {
        ...
        // THEN
        verify(exactly = 0) { onError.accept(any()) }
        verify { storageService.getPresignedUploadUrl(expectedServiceKey, expectedExpires, true) }
    }
    ...
    fun `PUT method returns success result`() {
        // GIVEN
        val path = StoragePath.fromString("uploads/photo.jpg")
        coEvery { storageService.getPresignedUploadUrl(any(), any(), any()) } returns testUrl
        val request = AWSS3StoragePathGetPresignedUrlRequest(
            path,
            expectedExpires,
            useAccelerateEndpoint = false,
